...
L_RATE = 1e-3
DIM_HID = 196
LAMBDA = 0.25 # regularization term
...
def training(epoch, learning_rate, reg_term, model):
    ...
    return (train_loss_ls), (valid_loss_ls), np.mean(valid_loss_ls), np.mean(valid_acc_ls)
...
def main(model, epoch=EPOCH, lr_start=L_RATE, reg_term=LAMBDA):
    ...
    for i in range(epoch):
        ...
        if i % DECAY_PER_EPOCH == 0: 
            lr *= DECAY_FACTOR
        train_loss, valid_loss, valid_loss_avg, valid_acc = training(i, lr, reg_term, model)
        acc_ls.append(valid_acc)
        val_loss_ls.append(valid_loss)
        train_loss_ls.append(train_loss)
        if valid_loss_avg <= val_loss_best and valid_acc >= acc_best:
            epoch_best = i
            acc_best = valid_acc
            val_loss_best = valid_loss_avg
            model_best = model
